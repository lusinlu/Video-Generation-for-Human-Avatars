...
class LTXVideoPipeline(DiffusionPipeline):
    ...
    def prepare_conditioning_version2(
        self,
        init_latents: torch.Tensor,
        ...
        """
        Prepare conditioning by summing image and pose latents, then lerping with noise.

        This version encodes both image and pose tensors through VAE, extends the image
        latent with zeros to match pose frames, sums them, and lerps with initial noise.
        No conditioning mask is used.

        Args:
            init_latents (torch.Tensor): The initial latent tensor of shape (b, c, f_l, h_l, w_l).
            image_tensor (torch.Tensor): Image tensor of shape (b, 3, 1, h, w).
            pose_tensor (torch.Tensor): Pose tensor of shape (b, 3, f, h, w).
            ...
            vae_per_channel_normalize (bool, optional): Whether to normalize channels during VAE encoding.
            strength (float, optional): Interpolation strength for lerp. Defaults to 1.0.

        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
                - init_latents (torch.Tensor): The updated and patchified latent tensor (b, n, c).
                - init_pixel_coords (torch.Tensor): Pixel coordinates for the latent tensor.
                - conditioning_mask (torch.Tensor): None (no mask used).
                - num_cond_latents (int): 0 (no extra conditioning latents).
        """
        assert isinstance(self.vae, CausalVideoAutoencoder)

        # Initialize conditioning mask (same as original)
        init_conditioning_mask = torch.zeros(
            init_latents[:, 0, :, :, :].shape,
            dtype=torch.float32,
            device=init_latents.device,
        ...
        # Encode image tensor through VAE
        # image_tensor: (b, 3, 1, h, w) -> image_latents: (b, c, 1, h_l, w_l)
        image_latents = vae_encode(
            image_tensor.to(dtype=self.vae.dtype, device=self.vae.device),
            self.vae,
            vae_per_channel_normalize=vae_per_channel_normalize,
        ).to(dtype=init_latents.dtype)

        # Encode pose tensor through VAE
        # pose_tensor: (b, 3, f, h, w) -> pose_latents: (b, c, f_l, h_l, w_l)
        pose_latents = vae_encode(
            pose_tensor.to(dtype=self.vae.dtype, device=self.vae.device),
            self.vae,
            vae_per_channel_normalize=vae_per_channel_normalize,
        ).to(dtype=init_latents.dtype)

        # Get dimensions
        b, c_l, f_l_pose, h_l, w_l = pose_latents.shape

        # Lerp with initial noise latents for the first f_l_pose frames
        init_latents[:, :, 0:1, :, :] = torch.lerp(
            init_latents[:, :, 0:1, :, :],  # noise (first arg)
            image_latents,  # conditioning (second arg)
            0.85,  # interpolation factor
        )
        init_latents[:, :, 1:, :, :] = torch.lerp(
            init_latents[:, :, 1:, :, :],  # noise (first arg)
            pose_latents[:, :, 1:, :, :],  # conditioning (second arg)
            strength,  # interpolation factor
        )

        # Update conditioning mask (same as original)
        init_conditioning_mask[:, 0, :, :] = 1
        ...
        # Patchify the conditioning mask (same as original)
        init_conditioning_mask, _ = self.patchifier.patchify(
            latents=init_conditioning_mask.unsqueeze(1)
        )
        init_conditioning_mask = init_conditioning_mask.squeeze(-1)

        # Return with conditioning_mask and 0 for num_cond_latents
        return init_latents, init_pixel_coords, init_conditioning_mask, 0
