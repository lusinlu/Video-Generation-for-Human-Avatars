...
def collate_latent_pairs(batch):
    ...
    audio_latents_padded = torch.zeros(
        batch_size, MAX_AUDIO_LENGTH, audio_dim, dtype=torch.float32
    )
...
class LatentPairDataset(Dataset):
    def __init__(
        ...
        # Collect encoder latent files (*.pt) and find matching audio
    ...
    def __getitem__(self, idx):
        rec = self.pairs[idx]
        audio_path, encoder_path = rec[0], rec[1]
