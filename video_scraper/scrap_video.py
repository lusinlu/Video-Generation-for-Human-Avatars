...
parser.add_argument(
    "--start_row", type=int, default=0, help="Start row index (inclusive)"
)
parser.add_argument(
    "--end_row", type=int, default=1000, help="End row index (exclusive)"
)
parser.add_argument(
    "--output_dir",
    type=Path,
...
def pre_filter_batch(csv_path, start_row, end_row, batch_size=10):
    ...
    while current_row < start_row + total_rows:
        ...
        for idx, row in batch_df.iterrows():
            ...
            print(
                f"Pre-filtering row {row_number} (batch {current_row}-{batch_end-1}): {ytid}"
            )
            ...
            try:
                ...
                subprocess.run(
                    f'ffmpeg -y -i "{preview_path}" -vn -ac 1 -ar 16000 "{audio_path}"',
                    shell=True,
                )
                ...
                if result["language"] != "en":
                    print(
                        f"  Failed language check (detected {result['language']}): {ytid}"
                    )
                    preview_path.unlink(missing_ok=True)
...
def transcribe_video(video_path):
    ...
    subprocess.run(
        f'ffmpeg -y -i "{video_path}" -vn -ac 1 -ar 16000 "{audio_path}"', shell=True
    )
    result = whisper_model.transcribe(audio_path)
    if result["language"] != "en":
...
def process_batch_pipeline():
        ...
        try:
            with open(args.transcripts_file, "r") as f:
                ...
                if isinstance(existing, list):
                    ...
                    print(
                        f"Loaded {len(all_data)} existing transcripts from {args.transcripts_file}"
                    )
        except Exception as e:
    ...
    try:
        for filtered_videos, batch_start, batch_end in pre_filter_batch(
            args.csv_path, args.start_row, args.end_row, batch_size=10
        ):
            batch_count += 1
